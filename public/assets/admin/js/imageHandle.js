...
/*!**************************************************!*\
  !*** ./resources/assets/admin/js/imageHandle.js ***!
  \**************************************************/
...
function formSubmit(e) {
  ...
  var bodyInput = document.getElementById('body'); // insert body content from div to text-area
...
function search_image(button) {
  var container = document.getElementById('founded-img'),
      ignorePhoto = container.querySelector('img'); // if the button is pressed while the image was received and viewed
  ...
  $.ajax({
    ...
    data: {
      ...
      // csrf
      q: document.getElementById('title').value,
      // query
      ignore: ignorePhoto
    },
    beforeSend: function beforeSend() {
      // on processing query
      var spinner = '<span class="spinner-border spinner-border-sm" role="status"></span> '; // make a spinner
...
function render_photo(photo) {
  ...
  var insertButton = document.createElement('button'); // create insert photo button
